...
{-- | create Handle --}
newHandle :: IO BL.Handle
newHandle = do
    let globalLevel = BL.Debug
    return BL.Handle {
        BL.log = \logMes str -> when (BL.level logMes >= globalLevel) $ do
            let levelMes = BL.level logMes
            currentTime <- BL.time logMes
