...
spec_attachmentToString = describe "Testing attachmentToString" $ do
    it "Should successfully convert Attachment with Doc to String" $ do
      let doc = Document {
            document_id = 781,
            document_ownerId = 129,
            document_title = "book.pdf",
            document_url = "https://server/link/222",
            document_accessKey = "x\n 0\n \NAK^IMYz.<E"
          }
          docAttach = defaultAttach {
            attach_type = "doc",
            attach_doc = Just doc
          }
    ...
    it "Should successfully convert Attachment with Photo to String" $ do
      let photo = Photo {
            photo_id = 810,
            photo_ownerId = 589,
            photo_accessKey = "<97z\vYG\v$L"
          }
          photoAttach = defaultAttach {
            attach_type = "photo",
            attach_photo = Just photo
          }
    ...
    it "Should successfully convert Attachment with Video to String" $ do
      let video = Video {
            video_id = 567,
            video_ownerId = 387,
            video_accessKey = "\DC47H)S:~:*3"
          }
          videoAttach = defaultAttach {
            attach_type = "video",
            attach_video = Just video
          }
    ...
    it "Should successfully convert Attachment with Audio to String" $ do
      let audio = Audio {
          ...
          audioAttach = defaultAttach {
            attach_type = "audio",
            attach_audio = Just audio
          }
...
spec_attachmentsToQuery = describe "Testing attachmentsToQuery" $ do
    let doc = Document {document_id = 781, 
          document_ownerId = 129,
          document_title = "book.pdf",
          document_url = "https://server/link/222",
          document_accessKey = "x\n 0\n \NAK^IMYz.<E"
    ...
    it "Should successfully convert [Attachment] with type \
       ...
       \if its type is not in ['doc','photo','video','audio'] to Query string" $ do
...
spec_returnStickerId = describe "Testing returnStickerId" $ do
    ...
    it "Should successfully (return StickerID) if its type is 'sticker'" $ do
      ...
      let stick = Sticker {
          ...
          stickAttach = defaultAttach {
            attach_type = "sticker",
            attach_sticker = Just stick
          }
...
spec_createEchoMessage = describe "Testing createEchoMessage" $ do
    let doc = Document {
        ...
        geo = Geo {
          geo_coordinates = "99.999 109.107"
        }
    ...
    it "Should successfully create Echo Message with Geo, with [Attachment]" $ do
      ...
      result `shouldBe` Identity "access_token=abcd0dcba&\
                                 \attachment=doc129_781%2Cphoto589_810_%3C97z%0BYG%0B%24L&\
                                 \lat=99.999&\
                                 \long=109.107&\
                                 \message=Hi%21&\
                                 \user_id=12345&\
...
spec_createRepeatMessage = describe "Testing createRepeatMessage" $ do
    it "Should successfully create Repeat Message" $ do
      ...
      result `shouldBe` Identity "access_token=abcd0dcba&\
                                 \message=How%20many%20replies%20do%20you%20prefer%20to%20recieve%3F&\
                                 \user_id=12345&\
                                 \v=5.81&\
                                 \keyboard="